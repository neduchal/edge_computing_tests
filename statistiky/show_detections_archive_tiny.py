...
def load_image_and_detections():
    img_path = filedialog.askopenfilename(title="Vyber obrázek")  # Zrušíme omezení na typ souboru
    if img_path:
        ...
        det_file1 = "./data/datasety/archive/test/labels/" + os.path.basename(img_path)[:-3] + "txt"
        det_file2 = "./data/output/output_tiny/" + os.path.basename(img_path) + ".txt"
        if det_file1 and det_file2:
            detections1 = load_yolo_detections_gt(det_file1)
