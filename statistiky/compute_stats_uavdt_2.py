    ...
    with open(pred_filename, "r") as pred_file:
        ...
        for line in lines:
            ...
            if (float(line_data[6]) > 0.5):
                all_pred_data.get(line_data[0]).get(str(file_number)).append(convert_bbox_to_relative([float(line_data[2]),float(line_data[3]),float(line_data[4]),float(line_data[5])], 1024, 544))
